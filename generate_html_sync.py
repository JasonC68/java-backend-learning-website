...
SECTION_RULES={
 ...
 "Spring":([("依赖注入","IoC/DI"),("控制反转","IoC/DI"),("依赖倒置","IoC/DI"),("循环依赖","循环依赖"),("三级缓存","循环依赖"),("动态代理","AOP"),("AOP","AOP"),("this调用","事务"),("事务","事务"),("扩展点","Bean"),("加载","Bean"),("销毁","Bean"),("Handler","SpringMVC"),("MVC","SpringMVC"),("DispatcherServlet","SpringMVC"),("请求","SpringMVC"),("约定大于配置","SpringBoot"),("启动器","SpringBoot"),("starter","SpringBoot"),("过滤器","SpringBoot"),("拦截器","SpringBoot"),("自动装配","SpringBoot"),("SpringBoot","SpringBoot"),("微服务","SpringCloud"),("负载均衡","SpringCloud"),("熔断","SpringCloud"),("降级","SpringCloud")],"Spring"),
...
ICONS = {
 ...
 "subadd": '<svg class="ic" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 5v6a3 3 0 0 0 3 3h8"/><polyline points="13 10 17 14 13 18"/></svg>',
...
.est .estseg{display:inline-flex;align-items:baseline;gap:7px;background:#f8fafc;border:1px solid #e5e7eb;border-radius:8px;padding:4px 12px;white-space:nowrap;font:inherit;cursor:pointer;transition:border-color .12s,box-shadow .12s}
.est .estseg:hover{border-color:#94a3b8;box-shadow:0 1px 3px rgba(0,0,0,.08)}
...
/* 复习池：跨板块加一批题统一复习。每个 chip 的颜色跟这道题自己所属的板块走（八股蓝/算法紫/项目粉），不受当前切换的模式影响 */
#poolBar{align-items:center;border:1px solid #e5e7eb;border-radius:10px;padding:8px 10px;background:#fafafa}
body.dark #poolBar{border-color:#3a3a3a;background:#1c1c1c}
.poolchips{display:inline-flex;flex-wrap:wrap;gap:6px}
.poolchip{display:inline-flex;align-items:center;gap:4px;border-radius:12px;padding:2px 6px 2px 10px;font-size:12px;cursor:pointer;white-space:nowrap}
.poolchip b{font-weight:600}
.poolchip .pcx{display:inline-flex;align-items:center;justify-content:center;cursor:pointer;padding:2px;border-radius:50%;width:14px;height:14px}
.poolchip .pcx svg{width:10px;height:10px}
#poolEmpty{font-size:12px;color:#bbb}
body.dark #poolEmpty{color:#555}
#poolEst{font-size:12px;color:#6b7280;white-space:nowrap}
body.dark #poolEst{color:#9ca3af}
.poolchip.pc-gu{background:#eff6ff;border:1px solid #dbeafe;color:#1d4ed8}
.poolchip.pc-gu .pcx{color:#93c5fd}
.poolchip.pc-gu .pcx:hover{color:#1d4ed8;background:#dbeafe}
body.dark .poolchip.pc-gu{background:#12233a;border:1px solid #1e3a5f;color:#93c5fd}
body.dark .poolchip.pc-gu .pcx{color:#3b82f6}
body.dark .poolchip.pc-gu .pcx:hover{color:#93c5fd;background:#1e3a5f}
.poolchip.pc-alg{background:#f5f3ff;border:1px solid #e9d5ff;color:#5b21b6}
.poolchip.pc-alg .pcx{color:#c4b5fd}
.poolchip.pc-alg .pcx:hover{color:#5b21b6;background:#ede9fe}
body.dark .poolchip.pc-alg{background:rgba(122,63,224,.18);border:1px solid #3a2a5c;color:#c4b5fd}
body.dark .poolchip.pc-alg .pcx{color:#a78bfa}
body.dark .poolchip.pc-alg .pcx:hover{color:#c4b5fd;background:#2a1f45}
.poolchip.pc-proj{background:#fdf2f8;border:1px solid #fbcfe8;color:#9d174d}
.poolchip.pc-proj .pcx{color:#f9a8d4}
.poolchip.pc-proj .pcx:hover{color:#9d174d;background:#fce7f3}
body.dark .poolchip.pc-proj{background:rgba(236,72,153,.15);border:1px solid #5f2640;color:#f9a8d4}
body.dark .poolchip.pc-proj .pcx{color:#ec4899}
body.dark .poolchip.pc-proj .pcx:hover{color:#f9a8d4;background:#3a1c2c}
body.dark #poolEmpty{color:#555}
...
.last{color:#6b7280;font-size:12px;white-space:nowrap}.q{line-height:1.7}
...
.qbtn{cursor:pointer;display:inline}
...
.blkctl .blkup .ic,.blkctl .blkdn .ic{width:10px;height:10px}
...
.studyblk-head .blkgrpdel{margin-left:auto;border:none;background:none;cursor:pointer;color:#9ca3af;padding:0 3px;line-height:1;display:inline-flex;align-items:center}
.studyblk-head .blkgrpdel:hover{color:#dc2626}
.studyblk-head .blkgrpdel .ic{width:14px;height:14px}
...
body.dark .qedit:hover{color:#60a5fa}
...
body.algmode .toolbar .btn.pri,body.algmode #bkModal .btn.pri,body.algmode #modal .btn.pri,body.algmode #cfModal .btn.pri,body.algmode #postponeModal .btn.pri{background:#7c3aed;border-color:#7c3aed;color:#fff}
body.dark.algmode .toolbar .btn.pri,body.dark.algmode #bkModal .btn.pri,body.dark.algmode #modal .btn.pri,body.dark.algmode #cfModal .btn.pri,body.dark.algmode #postponeModal .btn.pri{background:#7a3fe0;border-color:#7a3fe0}
body.algmode .toolbar .btn.pri:hover,body.algmode #bkModal .btn.pri:hover,body.algmode #modal .btn.pri:hover,body.algmode #cfModal .btn.pri:hover,body.algmode #postponeModal .btn.pri:hover{background:#6d28d9;border-color:#6d28d9}
body.dark.algmode .toolbar .btn.pri:hover,body.dark.algmode #bkModal .btn.pri:hover,body.dark.algmode #modal .btn.pri:hover,body.dark.algmode #cfModal .btn.pri:hover,body.dark.algmode #postponeModal .btn.pri:hover{background:#574095;border-color:#574095}
...
body.algmode .tag{color:#5b21b6;background:#f3edfb;border-color:#ddd6fe}
body.dark.algmode .tag{color:#c4b5fd;background:#262626;border-color:#3a3a3a}
body.algmode .tag.lc:hover{background:#7c3aed;color:#fff;border-color:#7c3aed}
body.dark.algmode .tag.lc:hover{background:#7a3fe0;border-color:#7a3fe0}
...
body.projmode .toolbar .btn.pri,body.projmode #bkModal .btn.pri,body.projmode #modal .btn.pri,body.projmode #cfModal .btn.pri,body.projmode #postponeModal .btn.pri{background:#db2777;border-color:#db2777;color:#fff}
body.dark.projmode .toolbar .btn.pri,body.dark.projmode #bkModal .btn.pri,body.dark.projmode #modal .btn.pri,body.dark.projmode #cfModal .btn.pri,body.dark.projmode #postponeModal .btn.pri{background:#ec4899;border-color:#ec4899}
body.projmode .toolbar .btn.pri:hover,body.projmode #bkModal .btn.pri:hover,body.projmode #modal .btn.pri:hover,body.projmode #cfModal .btn.pri:hover,body.projmode #postponeModal .btn.pri:hover{background:#be185d;border-color:#be185d}
body.dark.projmode .toolbar .btn.pri:hover,body.dark.projmode #bkModal .btn.pri:hover,body.dark.projmode #modal .btn.pri:hover,body.dark.projmode #cfModal .btn.pri:hover,body.dark.projmode #postponeModal .btn.pri:hover{background:#db2777;border-color:#db2777}
...
<div class="toolbar" id="poolBar">
  ...
  <span id="poolEst" class="poolest" title="按每题当前的建议专注时长（随复习次数递减）估算复习完这些题大概要多久"></span>
...
<div class="toolbar" id="dateBar">
  ...
  <button class="btn" id="postponeBtn" title="把所有任务（复习+学习）整体往后推迟一天" style="margin-left:6px">@@calendar@@ 推迟一天</button>
</div>
<div class="toolbar">
...
<div class="modal" id="poolDoneModal"><div class="card" style="max-width:360px">
  <h3 style="font-size:16px;margin-bottom:8px">@@checkcircle@@ 复习池完成</h3>
  <p id="poolDoneMsg" style="font-size:13px;color:#4b5563;margin-bottom:14px;line-height:1.6">复习池内容全部完成，要继续复习今天的任务，还是结束专注？</p>
  <div class="acts"><button class="btn" id="poolDoneStop">结束复习</button><button class="btn pri" id="poolDoneContinue">继续复习今日任务</button></div>
</div></div>
...
function highlightHTML(code){if(window.hljs){try{return hljs.highlight(code||"",{language:"java"}).value;}catch(e){try{return hljs.highlight("java",code||"").value;}catch(e2){try{return hljs.highlightAuto(code||"").value;}catch(e3){}}}}return esc(code);}
// 代码框：在光标处插入文本（用 execCommand 以保留浏览器原生撤销栈 Ctrl/Cmd+Z）
function codeInsert(ta,str){if(!document.execCommand||!document.execCommand("insertText",false,str)){const s=ta.selectionStart,e=ta.selectionEnd;ta.value=ta.value.slice(0,s)+str+ta.value.slice(e);ta.selectionStart=ta.selectionEnd=s+str.length;}}
// 回车自动缩进：与上一行一致；若在 { 后则多缩进一层；在 {} 之间回车则展开
function codeAutoIndent(ta){const v=ta.value,s=ta.selectionStart,e=ta.selectionEnd;
  const lineStart=v.lastIndexOf("\\n",s-1)+1;
  const indent=(v.slice(lineStart,s).match(/^[ \\t]*/)||[""])[0];
  const open=v.slice(0,s).replace(/[ \\t]*$/,"").slice(-1)==="{";
  const nextClose=v.charAt(e)==="}";
  if(open&&nextClose){codeInsert(ta,"\\n"+indent+"  \\n"+indent);ta.selectionStart=ta.selectionEnd=ta.selectionStart-(indent.length+1);}
  else if(open)codeInsert(ta,"\\n"+indent+"  ");
  else codeInsert(ta,"\\n"+indent);}
// 退格：光标正好在本行缩进末尾（内容最前）时，删掉换行+缩进，把本行内容并到上一行尾（空行则整行删除）
function codeBackDedent(ta){const v=ta.value,s=ta.selectionStart;
  if(s!==ta.selectionEnd||s<=0)return false;
  const lineStart=v.lastIndexOf("\\n",s-1)+1;
  if(lineStart===0)return false;
  const before=v.slice(lineStart,s);
  if(!(before.length>0&&/^[ \\t]+$/.test(before)&&!/[ \\t]/.test(v.charAt(s))))return false;
  ta.selectionStart=lineStart-1;ta.selectionEnd=s;
  if(!document.execCommand||!document.execCommand("delete")){ta.value=v.slice(0,lineStart-1)+v.slice(s);ta.selectionStart=ta.selectionEnd=lineStart-1;}
  return true;}
// Shift+Tab：删除当前行行首的一级缩进（最多 2 个空格或 1 个 Tab），保留撤销
function codeShiftDedent(ta){const v=ta.value,s=ta.selectionStart,e=ta.selectionEnd;
  const lineStart=v.lastIndexOf("\\n",s-1)+1;
  let n=0;while(n<2&&v.charAt(lineStart+n)===" ")n++;
  if(n===0&&v.charAt(lineStart)==="\\t")n=1;
  if(n===0)return false;
  ta.selectionStart=lineStart;ta.selectionEnd=lineStart+n;
  if(!document.execCommand||!document.execCommand("delete")){ta.value=v.slice(0,lineStart)+v.slice(lineStart+n);}
  const ns=s>=lineStart+n?s-n:(s>lineStart?lineStart:s),ne=e>=lineStart+n?e-n:(e>lineStart?lineStart:e);
  ta.selectionStart=ns;ta.selectionEnd=ne;return true;}
// 拼音等输入法拼字/上屏中不要拦截 Tab/Enter/Backspace，否则「打字中按回车直接上屏英文」会被我们当成换行处理，导致下面多出一行。
// e.isComposing/keyCode 229 在部分浏览器（尤其提交那一下 Enter）并不可靠，所以额外用 compositionstart/end 自己再记一份状态、并留一点缓冲时间。
function wireIMEGuard(ta){
  ta.addEventListener("compositionstart",()=>{ta.__composing=true;});
  ta.addEventListener("compositionend",()=>{ta.__composing=false;ta.__imeEndAt=Date.now();});
}
function isIMEComposing(e,ta){return !!(e.isComposing||e.keyCode===229||ta.__composing||(Date.now()-(ta.__imeEndAt||0)<30));}
function renderCodes(host,o,minOne){
  ...
  (o.codes||[]).forEach((c,idx)=>{
    ...
    if(c.fold){
      ...
      wireIMEGuard(ta);
      ta.onkeydown=e=>{if(isIMEComposing(e,ta))return;   // 拼音等输入法拼字/上屏中，交给输入法自己处理，不拦截
        if(e.key==="Tab"){e.preventDefault();if(e.shiftKey){if(codeShiftDedent(ta)){c.code=ta.value;paint();save();}}else{const s=ta.selectionStart,en=ta.selectionEnd;ta.value=ta.value.slice(0,s)+"  "+ta.value.slice(en);ta.selectionStart=ta.selectionEnd=s+2;c.code=ta.value;paint();save();}}};
...
// 学习板块：可有多个「学习板块(组)」，每组含若干「学习框/代码框」；可折叠、排序、删除；组内全删则组自动消失
...
function studyLastGroup(o){o.groups=o.groups||[];if(!o.groups.length)o.groups.push({blocks:[]});return o.groups[o.groups.length-1];}
// 代码答案框（算法题的默认答案就是它；不可删/不可移）
function mountAnsCode(host,o){host.innerHTML='<div class="codewrap ansbox"><pre aria-hidden="true"><code class="hljs"></code></pre><textarea spellcheck="false" placeholder="写你的代码答案…"></textarea></div>';
  const box=host.querySelector(".codewrap"),code=box.querySelector("code"),ta=box.querySelector("textarea");
  const paint=()=>{const v=ta.value;code.innerHTML=highlightHTML(v)+(v.slice(-1)==="\\n"?"\\n":"");};
  ta.value=o.ans||"";paint();ta.oninput=()=>{o.ans=ta.value;paint();save();};
  wireIMEGuard(ta);
  ta.onkeydown=e=>{if(isIMEComposing(e,ta))return;   // 拼音等输入法拼字/上屏中，交给输入法自己处理，不拦截
    if(e.key==="Tab"){e.preventDefault();if(e.shiftKey){if(codeShiftDedent(ta)){o.ans=ta.value;paint();save();}}else{codeInsert(ta,"  ");o.ans=ta.value;paint();save();}}else if(e.key==="Enter"){e.preventDefault();codeAutoIndent(ta);o.ans=ta.value;paint();save();}else if(e.key==="Backspace"&&codeBackDedent(ta)){e.preventDefault();o.ans=ta.value;paint();save();}};}
function renderOneBlock(host,b,ctl,cbs){
  if(b.t==="learn"){
    const box=document.createElement("div");box.className="learnwrap"+(b.fold?" folded":"");
    if(b.fold){const mf=plainFirstLine(b.html);box.innerHTML=ctl+'<div class="learn-folded">'+IC.book+' 学习'+(mf?'：'+esc(mf):'')+'</div>';box.querySelector(".learn-folded").onclick=e=>{if(e.target.closest(".blkctl"))return;cbs.fold();};}
    else box.innerHTML=ctl+'<div class="tui-learn"></div>';
    host.appendChild(box);wireBlkCtl(box,cbs.up,cbs.dn,cbs.fold,cbs.del);
    if(!b.fold)cbs.mount(box.querySelector(".tui-learn"),()=>b.html,v=>b.html=v,"写学习笔记…");
  }else{
    ...
    if(b.fold){const first=(b.code||"").split("\\n")[0]||"",more=(b.code||"").indexOf("\\n")>=0;
      ...
      box.querySelector("code").innerHTML=highlightHTML(first)+(more?' <span style="color:#888">…</span>':"");
      box.style.cursor="pointer";box.onclick=e=>{if(e.target.closest(".blkctl"))return;cbs.fold();};
    }else{
      ...
      wireIMEGuard(ta);
      ta.onkeydown=e=>{if(isIMEComposing(e,ta))return;   // 拼音等输入法拼字/上屏中，交给输入法自己处理，不拦截
        if(e.key==="Tab"){e.preventDefault();if(e.shiftKey){if(codeShiftDedent(ta)){b.code=ta.value;paint();save();}}else{codeInsert(ta,"  ");b.code=ta.value;paint();save();}}else if(e.key==="Enter"){e.preventDefault();codeAutoIndent(ta);b.code=ta.value;paint();save();}else if(e.key==="Backspace"&&codeBackDedent(ta)){e.preventDefault();b.code=ta.value;paint();save();}};
    }
    host.appendChild(box);wireBlkCtl(box,cbs.up,cbs.dn,cbs.fold,cbs.del);
  }}
function renderGroupBlocks(host,g,o,gi,mount){
  const clean=()=>{if(!g.blocks.length){o.groups.splice(gi,1);if(!o.groups.length)delete o.groups;}};
  g.blocks.forEach((b,idx)=>{
    const cbs={mount:mount,
      up:()=>{if(idx>0){const t=g.blocks[idx-1];g.blocks[idx-1]=g.blocks[idx];g.blocks[idx]=t;save();render();}},
      dn:()=>{if(idx<g.blocks.length-1){const t=g.blocks[idx+1];g.blocks[idx+1]=g.blocks[idx];g.blocks[idx]=t;save();render();}},
      fold:()=>{b.fold=!b.fold;save();render();},
      del:()=>{confirmDlg(b.t==="learn"?"删除这个学习框？":"删除这个代码框？",()=>{g.blocks.splice(idx,1);clean();save();render();});}};
    const ctl='<span class="blkctl"><button class="blkup" title="上移"'+(idx===0?' disabled':'')+'>'+IC.triup+'</button><button class="blkdn" title="下移"'+(idx===g.blocks.length-1?' disabled':'')+'>'+IC.tridown+'</button><button class="blkfd" title="折叠/展开">'+(b.fold?IC.chevronright:IC.chevrondown)+'</button><button class="blkdel" title="删除">✕</button></span>';
    renderOneBlock(host,b,ctl,cbs);
  });
  const az=document.createElement("div");az.className="blkadd";
  az.innerHTML='<div class="ba-btns"><button class="ba-learn">＋ 文本</button><button class="ba-code">＋ 代码</button></div>';
  az.querySelector(".ba-learn").onclick=()=>{g.blocks.push({t:"learn",html:""});save();render();};
  az.querySelector(".ba-code").onclick=()=>{g.blocks.push({t:"code",code:""});save();render();};
  host.appendChild(az);}
function renderStudyGroups(host,o,mount){host.innerHTML="";
  (o.groups||[]).forEach((g,gi)=>{
    const panel=document.createElement("div");panel.className="studyblk";
    panel.innerHTML='<div class="studyblk-head"><button class="blkfoldall" title="折叠/展开">'+(g.fold?IC.chevronright:IC.chevrondown)+'</button>'+IC.book+' 学习板块<button class="blkgrpdel" title="删除整个学习板块">'+IC.trash+'</button></div>'+(g.fold?'':'<div class="studyblk-body"></div>');
    host.appendChild(panel);
    panel.querySelector(".blkfoldall").onclick=e=>{e.stopPropagation();g.fold=!g.fold;save();render();};
    panel.querySelector(".blkgrpdel").onclick=e=>{e.stopPropagation();confirmDlg("删除整个学习板块？",()=>{o.groups.splice(gi,1);if(!o.groups.length)delete o.groups;save();render();});};
    if(!g.fold)renderGroupBlocks(panel.querySelector(".studyblk-body"),g,o,gi,mount);
  });}
// 迁移旧数据到新的「学习板块(组)」模型
function migrateStudy(o,isAlg){
  ...
  if(o.codes){o.codes=o.codes.map(c=>typeof c==="string"?{code:c}:c);
    if(isAlg){if(o.ans===undefined)o.ans=(o.codes[0]&&o.codes[0].code)||"";
      if(o.codes.length>1){o.groups=o.groups||[];o.groups.push({blocks:o.codes.slice(1).map(c=>({t:"code",code:c.code||"",fold:!!c.fold}))});}}
    else{o.blocks=o.blocks||[];o.codes.forEach(c=>o.blocks.push({t:"code",code:c.code||"",fold:!!c.fold}));}
    delete o.codes;}
  if(o.blocks){o.groups=o.groups||[];if(o.blocks.length)o.groups.push({blocks:o.blocks,fold:o.blkFold});delete o.blocks;delete o.blkFold;}
  if(isAlg&&o.ans===undefined)o.ans="";}
...
async function pull(force){if(restoring)return;if(!cfg){toast("请先配置云同步");return;}if(!force&&isEditing()){scheduleRetry();return;}try{setPill("拉取中…","busy");const c=await cloudGet();if(c&&typeof c==="object"){
    ...
    if(revOf(c)&&revOf(c)===revOf(state)){clearRetry();setPill("已拉取 "+nowt(),"ok");return;}   // 内容无变化：不重绘，避免打断输入
    if(!force&&isEditing()){setPill("正在输入，稍后拉取","busy");scheduleRetry();return;}          // 等待响应期间开始输入：推迟应用，别夺走光标
...
let focusOn=false,focusMode="gu",focusKindPref="review",focusSource="today",focusPos=-1,focusSeq=[],focusSeqI=0,focusTask=null,focusStartMs=0,focusElapsed=0,focusRunning=false,focusTick=null,focusAlerted=false;
const focusSkipped=new Set();
function focusMs(){return focusElapsed+(focusRunning?Date.now()-focusStartMs:0);}
// ---- 专注/计时器状态本地持久化（仅本机，不上云；刷新后自动恢复）----
const FUIKEY="focusui_v1";
function saveFocusUI(){try{localStorage.setItem(FUIKEY,JSON.stringify({
  t:{el:timerElapsed,run:timerRunning,st:timerStart},
  f:focusOn?{on:1,mode:focusMode,pref:focusKindPref,src:focusSource,pos:focusPos,task:focusTask,sk:[...focusSkipped],st:focusStartMs,el:focusElapsed,run:focusRunning,al:focusAlerted}:{on:0}
}));}catch(e){}}
function restoreFocusUI(){let d;try{d=JSON.parse(localStorage.getItem(FUIKEY)||"null");}catch(e){}if(!d)return;
  if(d.t){timerElapsed=d.t.el||0;timerStart=d.t.st||Date.now();timerRunning=!!d.t.run;
    const btn=document.getElementById("timerToggle"),box=document.getElementById("timerBox");
    if(timerRunning){if(btn){btn.innerHTML=IC.pause;btn.title="暂停";btn.classList.add("on");}if(box)box.classList.add("run");if(timerTick)clearInterval(timerTick);timerTick=setInterval(renderTimer,250);}
    renderTimer();}
  if(d.f&&d.f.on&&d.f.task){focusOn=true;focusMode=d.f.mode;focusKindPref=d.f.pref;focusSource=d.f.src;focusPos=(d.f.pos!==undefined?d.f.pos:-1);focusTask=d.f.task;focusSkipped.clear();(d.f.sk||[]).forEach(x=>focusSkipped.add(x));focusStartMs=d.f.st||Date.now();focusElapsed=d.f.el||0;focusRunning=!!d.f.run;focusAlerted=!!d.f.al;
    document.getElementById("focusPanel").style.display="";document.getElementById("focusBtn").classList.add("pri");
    const t=focusTask,k=document.getElementById("focusKind");k.innerHTML=(t.kind==="review"?IC.review+" 复习":IC.book+" 新学");k.className="fp-kind "+t.kind;
    ...
    const pb=document.getElementById("focusPause");if(pb)pb.innerHTML=focusRunning?(IC.pause+" 暂停"):(IC.play+" 继续");
...
function loadFocusTask(t){focusTask=t;focusStartMs=Date.now();focusElapsed=0;focusRunning=true;focusAlerted=false;
  ...
  document.getElementById("focusQ").textContent=(t.sec?("["+t.sec+"] "):"")+t.q;
  document.getElementById("focusAllot").textContent="/ 建议 "+fmtMS(focusMinFor(t)*60000);
  if(focusTick)clearInterval(focusTick);focusTick=setInterval(renderFocus,250);renderFocus();saveFocusUI();
...
function focusNext(){
  ...
  const all=focusQueue(focusMode).filter(t=>!focusSkipped.has(t.id)).map(t=>Object.assign({},t,{_p:pos[t.id]!==undefined?pos[t.id]:1e9}));
  // 从当前题位置往下取下一个到期题；到底后再回到最靠前的到期题（保证今日任务都能覆盖）
  const pick=a=>{const after=a.filter(t=>t._p>focusPos).sort((x,y)=>x._p-y._p);if(after.length)return after[0];const s=a.slice().sort((x,y)=>x._p-y._p);return s[0]||null;};
  const wantKind=focusKindPref==="new"?"new":"review";
  let nx=pick(all.filter(t=>t.kind===wantKind));
  if(!nx){const other=all.filter(t=>t.kind!==wantKind);
    if(other.length){focusKindPref=wantKind==="new"?"review":"new";toast(wantKind==="new"?"新学已全部完成，转入复习专注":"复习已全部完成，转入新学专注");nx=pick(other);}}
...
function focusFromItem(id){
  ...
  focusMode=mode;const kind=(get(id).cnt||0)>0?"review":"new";
  focusSkipped.clear();focusSource="today";focusKindPref=kind;
  const meta=focusMetaOf(id,kind);if(!meta){toast("没有可专注的题");return;}
  const pos=focusOrderIndex();focusPos=(pos[id]!==undefined?pos[id]:-1);   // 之后从这题位置往下继续
  focusOn=true;document.getElementById("focusPanel").style.display="";document.getElementById("focusBtn").classList.add("pri");loadFocusTask(meta);jumpToFocusItem();}
...
function startFocus(pref){focusSkipped.clear();focusMode=mode;focusSource="today";focusKindPref=(pref==="new")?"new":"review";focusPos=-1;if(!focusQueue(focusMode).length){toast("今日任务已全部完成");return;}
...
function renderPool(){
  ...
  box.innerHTML=valid.map(id=>{const m=poolMeta(id);const qs=m.q.length>10?m.q.slice(0,10)+"…":m.q;
    const kc=m.isAlg?"pc-alg":(m.isProj?"pc-proj":"pc-gu");   // chip 颜色跟这道题自己所属的板块（八股/算法/项目）走，不受当前切换的模式影响
    return '<span class="poolchip '+kc+'" data-id="'+id+'" title="'+esc(m.q)+'"><b>'+esc(poolSecAbbr(m.sec))+'</b>·'+esc(qs)+'<i class="pcx" data-id="'+id+'" title="移出复习池">'+IC.x+'</i></span>';}).join("");
  const eb=document.getElementById("poolEmpty");if(eb)eb.style.display=valid.length?"none":"";
  const est=document.getElementById("poolEst");
  if(est){if(valid.length){const totalMin=valid.reduce((s,id)=>s+focusMinFor(poolMeta(id)),0);est.textContent="预计 "+fmtDur(totalMin);est.style.display="";}else{est.style.display="none";}}
...
function focusPoolNext(){
  ...
  if(!ids.length){
    const skipped=focusSkipped.size;
    focusSetRun(false);   // 弹窗选择期间先暂停计时，避免面板还显示已完成那一题、时间却继续在走
    const m=document.getElementById("poolDoneMsg");
    if(m)m.textContent=skipped?"复习池里剩下的都跳过了，要继续复习今天的任务，还是结束专注？":"复习池内容全部完成，要继续复习今天的任务，还是结束专注？";
    document.getElementById("poolDoneModal").classList.add("show");
    return;}
  const meta=poolMeta(ids[0]);
  if(!meta){removeFromPool(ids[0]);focusPoolNext();return;}
  focusMode=meta.isAlg?"alg":(meta.isProj?"proj":"gu");
  loadFocusTask(meta);}
function poolDoneContinueToday(){
  document.getElementById("poolDoneModal").classList.remove("show");
  focusSource="today";focusMode=mode;focusSkipped.clear();focusPos=-1;
  if(!focusQueue(focusMode).length){toast("今日任务已全部完成");endFocus();return;}
  focusNext();}
...
document.getElementById("poolDoneStop").onclick=()=>{document.getElementById("poolDoneModal").classList.remove("show");endFocus();};
document.getElementById("poolDoneContinue").onclick=poolDoneContinueToday;
...
function renderAlg(tb){
  ...
  list.forEach(it=>{const st=get(it.id);
    ...
    if(opened){
      ...
      bar+='<button class="ebtn addgroup">＋ 学习</button><button class="ebtn addtext">＋ 文本</button><button class="ebtn addcode">＋ 代码</button>';
      bar+='</div>';
      let body='';
      if(o.memoOn){
        if(o.memoHide){const mf=plainFirstLine(o.memo);body+='<div class="memo-folded"><button class="memofold memobtn">'+IC.chevronright+'</button><button class="memodel memobtn">✕</button>'+IC.bulb+' 助记'+(mf?'：'+esc(mf):'')+'</div>';}
        else body+='<div class="memo-label">'+IC.bulb+' 助记</div><div class="memowrap"><button class="memofold memobtn" title="隐藏助记">'+IC.chevrondown+'</button><button class="memodel memobtn" title="删除助记">✕</button><div class="tui-memo"></div></div>';
      }
      if(o.ansHide)body+='<div class="note-folded"><button class="notefold notebtn" title="显示答案">'+IC.chevronright+'</button>'+IC.note+' 答案已隐藏</div>';
      else body+='<div class="notewrap"><button class="notefold notebtn" title="隐藏答案">'+IC.chevrondown+'</button><div class="anscode"></div></div>';
      body+='<div class="studygroups"></div>';
      ...
      const anw=td.querySelector(".notewrap");if(anw)anw.querySelector(".notefold").onclick=e=>{e.stopPropagation();o.ansHide=true;save();render();};
      const afold=td.querySelector(".note-folded");if(afold)afold.onclick=()=>{o.ansHide=false;save();render();};
      td.querySelector(".addgroup").onclick=()=>{o.groups=o.groups||[];o.groups.push({blocks:[{t:"learn",html:""}]});save();render();};
      td.querySelector(".addtext").onclick=()=>{studyLastGroup(o).blocks.push({t:"learn",html:""});save();render();};
      td.querySelector(".addcode").onclick=()=>{studyLastGroup(o).blocks.push({t:"code",code:""});save();render();};
      er.appendChild(td);tb.appendChild(er);
      if(o.memoOn&&!o.memoHide)mountEd(td.querySelector(".tui-memo"),()=>o.memo,v=>o.memo=v,"写思路/口诀…");
      if(!o.ansHide)mountAnsCode(td.querySelector(".anscode"),o);
      if(o.groups&&o.groups.length)renderStudyGroups(td.querySelector(".studygroups"),o,mountEd);
    }
  });
  ...
  secs.forEach(s=>{
    ...
    list.forEach(it=>{const st=get(it.id);
      ...
      if(opened){
        ...
        if(o.noteHide)body+='<div class="note-folded"><button class="notefold notebtn" title="显示答案">'+IC.chevronright+'</button>'+IC.note+' 答案已隐藏</div>';
        else body+='<div class="notewrap"><button class="notefold notebtn" title="隐藏答案">'+IC.chevrondown+'</button><div class="tui"></div></div>';
        body+='<div class="studygroups"></div>';
        td.innerHTML=bar+body;
        const addb=td.querySelector(".addmemo");if(addb)addb.onclick=()=>{o.memoOn=true;o.memoHide=false;save();render();};
        const delMemo=()=>confirmDlg("删除助记？",()=>{o.memoOn=false;delete o.memo;o.memoHide=false;save();render();});
        ...
        if(o.groups&&o.groups.length)renderStudyGroups(td.querySelector(".studygroups"),o,mount);
      }});
...
document.querySelectorAll("#trimModal [data-trim]").forEach(btn=>btn.onclick=()=>{
  ...
  const moving=list.slice().sort((a,b)=>(b.when||"").localeCompare(a.when||"")).slice(0,toMove);  // 优先缩减「安排日期最晚」的任务，逾期越久的越先保留复习
...
function applyTheme(){const m=localStorage.getItem("theme")||"system";const dark=m==="dark"||(m==="system"&&window.matchMedia&&matchMedia("(prefers-color-scheme: dark)").matches);document.body.classList.toggle("dark",dark);document.querySelectorAll("[data-theme]").forEach(b=>b.classList.toggle("on",b.dataset.theme===m));}
document.querySelectorAll("[data-theme]").forEach(b=>b.onclick=()=>{localStorage.setItem("theme",b.dataset.theme);applyTheme();});
...
pm_js = open("/tmp/tui/md.bundle.js", encoding="utf-8").read()
# 给编辑器加 Cmd/Ctrl+Y：把选中文字切换为行内代码（extraKeys 在插件链最前，优先级高于默认重做绑定）
if "codeToggle" not in pm_js:
    pm_js = pm_js.replace(
        "var toggleHL = toggleMark(schema2.marks.highlight);",
        "var toggleHL = toggleMark(schema2.marks.highlight);var codeToggle = schema2.marks.code ? toggleMark(schema2.marks.code) : function(){return false;};", 1)
    pm_js = pm_js.replace('"Mod-e": toggleHL,', '"Mod-e": toggleHL,"Mod-y": codeToggle,"Mod-Y": codeToggle,', 1)
pm_js = pm_js.replace("</script", "<\\/script")
