...
<div class="sub"><span style="color:#9ca3af">v2.7.11</span></div>
...
let secFilter="all",lvlFilter="all",dateFilter="all",pickedDate="",starOnly=false,recycleMode=false,timer=null,openIds=new Set(),editors=[],dirty=false,retryTimer=null,retryN=0,stuckToday=new Set(),stuckDay="",restoring=false;
...
async function cloudGet(){const r=await fetch(url()+"/latest",{headers:{"X-Master-Key":cfg.key}});if(!r.ok)throw new Error("HTTP "+r.status);return (await r.json()).record;}
async function cloudPut(){const r=await fetch(url(),{method:"PUT",headers:{"Content-Type":"application/json","X-Master-Key":cfg.key},body:JSON.stringify(state)});if(!r.ok)throw new Error("HTTP "+r.status);}
function scheduleRetry(){if(retryTimer)return;if(retryN>=5)return;const d=Math.min(60000,10000*Math.pow(2,retryN));retryN++;retryTimer=setTimeout(()=>{retryTimer=null;autoSync();},d);}
function clearRetry(){retryN=0;if(retryTimer){clearTimeout(retryTimer);retryTimer=null;}}
...
async function push(){if(!cfg)return;try{setPill("同步中…","busy");await cloudPut();dirty=false;clearRetry();setPill("已同步 "+nowt(),"ok");}catch(e){setPill("上传失败："+(e&&e.message||"网络错误"),"warn");scheduleRetry();}}
