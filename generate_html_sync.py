...
<div class="sub"><span style="color:#9ca3af">v2.11.4.5</span></div>
...
function saveFocusUI(){try{localStorage.setItem(FUIKEY,JSON.stringify({
  ...
  f:focusOn?{on:1,mode:focusMode,pref:focusKindPref,src:focusSource,seq:focusSeq,si:focusSeqI,task:focusTask,sk:[...focusSkipped],st:focusStartMs,el:focusElapsed,run:focusRunning,al:focusAlerted}:{on:0}
}));}catch(e){}}
function restoreFocusUI(){let d;try{d=JSON.parse(localStorage.getItem(FUIKEY)||"null");}catch(e){}if(!d)return;
  if(d.t){timerElapsed=d.t.el||0;timerStart=d.t.st||Date.now();timerRunning=!!d.t.run;
    ...
    renderTimer();}
  if(d.f&&d.f.on&&d.f.task){focusOn=true;focusMode=d.f.mode;focusKindPref=d.f.pref;focusSource=d.f.src;focusSeq=d.f.seq||[];focusSeqI=d.f.si||0;focusTask=d.f.task;focusSkipped.clear();(d.f.sk||[]).forEach(x=>focusSkipped.add(x));focusStartMs=d.f.st||Date.now();focusElapsed=d.f.el||0;focusRunning=!!d.f.run;focusAlerted=!!d.f.al;
