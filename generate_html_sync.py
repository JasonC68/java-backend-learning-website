...
<div class="sub"><span style="color:#9ca3af">v2.6.1</span></div>
...
// ---- 云端备份（独立 Bin，自动创建，跨设备）----
async function ensureBkBin(){if(state.__bkBin)return state.__bkBin;
  const r=await fetch("https://api.jsonbin.io/v3/b",{method:"POST",headers:{"Content-Type":"application/json","X-Master-Key":cfg.key,"X-Bin-Name":"打卡备份","X-Bin-Private":"true"},body:JSON.stringify({backups:[]})});
  if(!r.ok)throw 0;const j=await r.json();state.__bkBin=j.metadata.id;save();return state.__bkBin;}
async function cloudBkGet(){const id=await ensureBkBin();const r=await fetch("https://api.jsonbin.io/v3/b/"+id+"/latest",{headers:{"X-Master-Key":cfg.key}});if(!r.ok)throw 0;const j=await r.json();return (j.record&&j.record.backups)||[];}
async function cloudBkPut(arr){const id=await ensureBkBin();const r=await fetch("https://api.jsonbin.io/v3/b/"+id,{method:"PUT",headers:{"Content-Type":"application/json","X-Master-Key":cfg.key},body:JSON.stringify({backups:arr})});if(!r.ok)throw 0;}
async function renderCloud(){const box=document.getElementById("bkCloudList");
  if(!cfg){box.innerHTML='<div style="color:#9ca3af;font-size:13px;padding:8px">配置「☁️ 云同步」后可用</div>';return;}
  box.innerHTML='<div style="color:#9ca3af;font-size:13px;padding:8px">加载中…</div>';
  let a;try{a=await cloudBkGet();}catch(e){box.innerHTML='<div style="color:#dc2626;font-size:13px;padding:8px">云端读取失败，检查网络/配置</div>';return;}
  if(!a.length){box.innerHTML='<div style="color:#9ca3af;font-size:13px;padding:8px">还没有云端备份</div>';return;}
  box.innerHTML=a.map((b,i)=>'<div class="bk-item"><div class="meta"><div>'+(esc(b.label)||"备份")+'</div><div class="t">'+fmtTs(b.ts)+' · '+progCount(b.data)+' 题有记录</div></div><button class="rb" data-i="'+i+'">恢复</button><button class="db" data-i="'+i+'">删除</button></div>').join("");
  box.querySelectorAll(".rb").forEach(btn=>btn.onclick=async()=>{const i=+btn.dataset.i;if(confirm("恢复到这个云端备份？当前进度会被覆盖")){try{const arr=await cloudBkGet();state=JSON.parse(arr[i].data);save();render();bkModal.classList.remove("show");}catch(e){alert("恢复失败");}}});
  box.querySelectorAll(".db").forEach(btn=>btn.onclick=async()=>{const i=+btn.dataset.i;try{const arr=await cloudBkGet();arr.splice(i,1);await cloudBkPut(arr);renderCloud();}catch(e){alert("删除失败");}});
}
...
document.getElementById("bkNewCloud").onclick=async()=>{if(!cfg){alert("请先配置「☁️ 云同步」");return;}const btn=document.getElementById("bkNewCloud");btn.disabled=true;btn.textContent="备份中…";try{const arr=await cloudBkGet();arr.unshift({ts:Date.now(),label:document.getElementById("bkLabel").value.trim(),data:JSON.stringify(state)});if(arr.length>15)arr.length=15;await cloudBkPut(arr);document.getElementById("bkLabel").value="";renderCloud();}catch(e){alert("云端备份失败，检查网络/配置");}btn.disabled=false;btn.textContent="☁️ 云端备份";};
