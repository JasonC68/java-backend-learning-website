...
<div class="sub"><span style="color:#9ca3af">v3.0.1.1</span></div>
...
function mountAnsCode(host,o){host.innerHTML='<div class="codewrap ansbox"><pre aria-hidden="true"><code class="hljs"></code></pre><textarea spellcheck="false" placeholder="写你的代码答案…"></textarea></div>';
  ...
  ta.onkeydown=e=>{if(e.key==="Tab"){e.preventDefault();codeInsert(ta,"  ");o.ans=ta.value;paint();save();}else if(e.key==="Enter"){e.preventDefault();codeAutoIndent(ta);o.ans=ta.value;paint();save();}else if(e.key==="Backspace"&&codeBackDedent(ta)){e.preventDefault();o.ans=ta.value;paint();save();}};}
function renderOneBlock(host,b,ctl,cbs){
  if(b.t==="learn"){
    ...
    if(b.fold){const first=(b.code||"").split("\\n")[0]||"",more=(b.code||"").indexOf("\\n")>=0;
      ...
      ta.onkeydown=e=>{if(e.key==="Tab"){e.preventDefault();codeInsert(ta,"  ");b.code=ta.value;paint();save();}else if(e.key==="Enter"){e.preventDefault();codeAutoIndent(ta);b.code=ta.value;paint();save();}else if(e.key==="Backspace"&&codeBackDedent(ta)){e.preventDefault();b.code=ta.value;paint();save();}};
    }
    host.appendChild(box);wireBlkCtl(box,cbs.up,cbs.dn,cbs.fold,cbs.del);
  }}
