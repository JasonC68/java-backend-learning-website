...
<div class="sub"><span style="color:#9ca3af">v2.10.5</span></div>
...
let secFilter="all",lvlFilter="all",diffFilter="all",dateFilter="all",pickedDate="",starOnly=false,recycleMode=false,timer=null,openIds=new Set(),editors=[],dirty=false,retryTimer=null,stuckToday=new Set(),stuckDay="",restoring=false;
...
document.getElementById("bkNewCloud").onclick=async()=>{if(!cfg){toast("请先配置「☁️ 云同步」");return;}const btn=document.getElementById("bkNewCloud");btn.disabled=true;btn.textContent="备份中…";
  try{const snap=Object.assign({},state);delete snap.__bkIndex;delete snap.__backups;
    ...
    const r=await fetch(api("checkin"),{method:"POST",headers:H({"Content-Type":"application/json","Prefer":"return=minimal"}),body:JSON.stringify([{id:rid,kind:"backup",label:document.getElementById("bkLabel").value.trim(),data:snap}])});if(!r.ok)throw 0;
    const a=bkIndex();a.unshift({ts:Date.now(),label:document.getElementById("bkLabel").value.trim(),bin:rid});if(a.length>15)a.length=15;
