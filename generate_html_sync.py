...
<div class="sub"><span style="color:#9ca3af">v2.6.3</span></div>
...
// ---- 云端备份：存进进度数据(__backups)，随现有云同步跨设备 ----
function cloudList(){return state.__backups||(state.__backups=[]);}
function renderCloud(){const box=document.getElementById("bkCloudList");const a=cloudList();
  if(!a.length){box.innerHTML='<div style="color:#9ca3af;font-size:13px;padding:8px">还没有云端备份，点上方「☁️ 云端备份」创建'+(cfg?'':'（未配置云同步，先存本地，配置后自动上云）')+'</div>';return;}
  ...
  box.querySelectorAll(".rb").forEach(btn=>btn.onclick=()=>{const i=+btn.dataset.i;if(confirm("恢复到这个云端备份？当前进度会被覆盖")){try{const keep=cloudList();const snap=JSON.parse(cloudList()[i].data);snap.__backups=keep;state=snap;save();render();bkModal.classList.remove("show");}catch(e){alert("恢复失败");}}});
  box.querySelectorAll(".db").forEach(btn=>btn.onclick=()=>{const i=+btn.dataset.i;cloudList().splice(i,1);save();renderCloud();});
}
...
document.getElementById("bkNewCloud").onclick=()=>{const snap=Object.assign({},state);delete snap.__backups;const a=cloudList();a.unshift({ts:Date.now(),label:document.getElementById("bkLabel").value.trim(),data:JSON.stringify(snap)});if(a.length>10)a.length=10;document.getElementById("bkLabel").value="";save();renderCloud();};
