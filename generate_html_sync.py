...
<div class="sub"><span style="color:#9ca3af">v2.11.8.0</span></div>
...
// 代码框回车自动缩进：与上一行一致；若在 { 后则多缩进一层；在 {} 之间回车则展开
function codeAutoIndent(ta){const v=ta.value,s=ta.selectionStart,e=ta.selectionEnd;
  ...
  let ins,caret;
  if(open&&nextClose){ins="\\n"+indent+"  \\n"+indent;caret=s+3+indent.length;}
  else if(open){ins="\\n"+indent+"  ";caret=s+ins.length;}
  else{ins="\\n"+indent;caret=s+ins.length;}
  ta.value=v.slice(0,s)+ins+v.slice(e);ta.selectionStart=ta.selectionEnd=caret;}
...
function mountAnsCode(host,o){host.innerHTML='<div class="codewrap ansbox"><pre aria-hidden="true"><code class="hljs"></code></pre><textarea spellcheck="false" placeholder="写你的代码答案…"></textarea></div>';
  ...
  ta.onkeydown=e=>{if(e.key==="Tab"){e.preventDefault();const s=ta.selectionStart,en=ta.selectionEnd;ta.value=ta.value.slice(0,s)+"  "+ta.value.slice(en);ta.selectionStart=ta.selectionEnd=s+2;o.ans=ta.value;paint();save();}else if(e.key==="Enter"){e.preventDefault();codeAutoIndent(ta);o.ans=ta.value;paint();save();}};}
function renderOneBlock(host,b,ctl,cbs){
  if(b.t==="learn"){
    ...
    if(b.fold){const first=(b.code||"").split("\\n")[0]||"",more=(b.code||"").indexOf("\\n")>=0;
      ...
      ta.onkeydown=e=>{if(e.key==="Tab"){e.preventDefault();const s=ta.selectionStart,en=ta.selectionEnd;ta.value=ta.value.slice(0,s)+"  "+ta.value.slice(en);ta.selectionStart=ta.selectionEnd=s+2;b.code=ta.value;paint();save();}else if(e.key==="Enter"){e.preventDefault();codeAutoIndent(ta);b.code=ta.value;paint();save();}};
    }
    host.appendChild(box);wireBlkCtl(box,cbs.up,cbs.dn,cbs.fold,cbs.del);
  }}
