...
<div class="sub"><span style="color:#9ca3af">v2.9.4</span></div>
...
function render(){const tb=document.getElementById("tb");
  ...
  secs.forEach(s=>{
    ...
    list.forEach(it=>{const st=get(it.id);
      ...
      tr.querySelector(".lvl").onclick=()=>{st.lvl=(st.lvl+1)%4;if(st.cnt>0&&st.last===today())st.next=schedNext(st.cnt,st.lvl);save();render();};
