...
<div class="sub"><span style="color:#9ca3af">v2.6.2</span></div>
...
async function ensureBkBin(){if(state.__bkBin)return state.__bkBin;
  const r=await fetch("https://api.jsonbin.io/v3/b",{method:"POST",headers:{"Content-Type":"application/json","X-Master-Key":cfg.key,"X-Bin-Name":"checkin-backups"},body:JSON.stringify({backups:[]})});
  if(!r.ok)throw 0;const j=await r.json();state.__bkBin=j.metadata.id;save();return state.__bkBin;}
async function cloudBkGet(){if(!state.__bkBin)return [];const r=await fetch("https://api.jsonbin.io/v3/b/"+state.__bkBin+"/latest",{headers:{"X-Master-Key":cfg.key}});if(!r.ok)throw 0;const j=await r.json();return (j.record&&j.record.backups)||[];}
