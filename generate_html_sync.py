...
let focusOn=false,focusMode="gu",focusTask=null,focusStartMs=0,focusElapsed=0,focusRunning=false,focusTick=null,focusAlerted=false;
...
function focusNext(){const q=focusQueue(focusMode).filter(t=>!focusSkipped.has(t.id));
...
function startFocus(){focusSkipped.clear();focusMode=mode;if(!focusQueue(focusMode).length){toast("🎉 今日任务已全部完成");return;}
