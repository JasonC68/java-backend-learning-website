...
function renderCodes(host,o,minOne){
  ...
  (o.codes||[]).forEach((c,idx)=>{
    ...
    if(c.fold){
      ...
      ta.onkeydown=e=>{if(e.key==="Tab"){e.preventDefault();if(e.shiftKey){if(codeShiftDedent(ta)){c.code=ta.value;paint();save();}}else{const s=ta.selectionStart,en=ta.selectionEnd;ta.value=ta.value.slice(0,s)+"  "+ta.value.slice(en);ta.selectionStart=ta.selectionEnd=s+2;c.code=ta.value;paint();save();}}};
