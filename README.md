...
- **修订号 +1**：小改动（样式、文案、小修复），如 2.1.0 → 2.1.1
- **次版本 +1**：较大改动 / 新增功能，如 2.0.1 → 2.1.0
- **主版本 +1**：整体界面换风格 / 重构，如 2.x.x → 3.0.0

版本号显示在页面副标题处，方便确认部署的是最新版。

### 版本历史（要点）

- **v2.1.1** — 新增「📌 今天任务」筛选（今天打卡 + 今天到期复习合并）
- **v2.1.0** — 题目加星收藏 + 「仅看收藏」筛选
- **v2.0.1** — 编辑器宽度改为随列表自适应，图片不溢出
- **v2.0.0** — 精简界面文案；编辑器固定宽度、图片限宽
- v1 系列 — 自定义日历（带打卡红点）、艾宾浩斯复习、今天/明天/指定日期筛选、AI 题板块、每天约 8 题计划、ProseMirror 所见即所得编辑器、云同步自动拉取与重试等逐步迭代
